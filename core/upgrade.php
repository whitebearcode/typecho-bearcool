...
$UpgradePassword= new Typecho_Widget_Helper_Form_Element_Text('UpgradePassword', null, '', '升级密钥', '请自行填入升级密钥,建议复杂点,在线升级时为保证安全，需要该密钥进行鉴权。');
    ...
    if (!empty($options->UpgradePassword)){
        ...
        $files = $options->siteUrl.'/usr/themes/bearcool/vendors/Upgrade/Upgrade.php';
            ...
            ajax({
			    ...
			    success:function(data){
			    	if(data.status==200 && data.code==1){
			    		Swal.fire("BearCool主题升级成功！", data.message, "success").then((isConfirm) => window.location.reload());
			    	}
